...
function toOrderResponse(ordem) {
  return {
    ...
    usuarioId: ordem.usuarioId ? String(ordem.usuarioId) : null,
...
/**
 * Livro legado e compatível:
 * GET /ordens/:clubeId
 *
 * Retorna os dois formatos:
 * - compra/venda  -> compatibilidade antiga
 * - compras/vendas -> compatibilidade nova
 */
router.get('/:clubeId', async (req, res) => {
  try {
    ...
    return res.json({
      ...
      ultimoPreco: clube
        ? round2(clube.precoAtual != null ? clube.precoAtual : clube.preco)
        : null,
    });
  } catch (err) {
  ...
  try {
    ...
    const ordemOut = await runTx({
      ...
      mutate: async (session) => {
        ...
        if (
          Number(ordem.restante || 0) <= 0 ||
          String(ordem.status || '').toLowerCase() === 'cancelada' ||
          String(ordem.status || '').toLowerCase() === 'executada'
        ) {
    ...
    if (err?.code === 'ALREADY') {
      return res.status(400).json({
        erro: 'Não é possível cancelar uma ordem já executada/cancelada.',
      });
    }

    return res
      .status(Number(err?.status || 500))
      .json({ erro: 'Erro interno ao cancelar ordem.' });
  }
});
