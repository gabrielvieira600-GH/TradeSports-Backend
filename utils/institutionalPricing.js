...
function pricesFor(state, baseValue) {
  return {
    ask: tickUp(Number(baseValue) * (1 + Number(state.sellMarginPct || 0))),
    bid: tickDown(Number(baseValue) * (1 - Number(state.buyDiscountPct || 0))),
  };
}

module.exports = { TICK_SIZE, round2, tickUp, tickDown, pricesFor };
